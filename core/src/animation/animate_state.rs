...
macro_rules! impl_animate_state_for_writers {
  () => {
    #[inline]
...
impl<V: Clone + 'static> AnimateStateSetter for Writer<V> {
  type Value = V;

  impl_animate_state_for_writers!();
}

impl<V: Clone + 'static> AnimateStateSetter for Stateful<V> {
  type Value = V;

  impl_animate_state_for_writers!();
}

impl<V: Clone + 'static> AnimateStateSetter for PartWriter<V> {
  type Value = V;

  impl_animate_state_for_writers!();
}

impl<V: Clone + Lerp + 'static> AnimateState for Writer<V> {
  fn calc_lerp_value(&mut self, from: &V, to: &V, rate: f32) -> V { from.lerp(to, rate) }
}

impl<V: Clone + Lerp + 'static> AnimateState for Stateful<V> {
  fn calc_lerp_value(&mut self, from: &V, to: &V, rate: f32) -> V { from.lerp(to, rate) }
}

impl<V: Clone + Lerp + 'static> AnimateState for PartWriter<V> {
  fn calc_lerp_value(&mut self, from: &V, to: &V, rate: f32) -> V { from.lerp(to, rate) }
}
