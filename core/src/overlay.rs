...
impl OverlayState {
  ...
  fn show(&self, w: impl FnWidget + 'static, style: Option<OverlayStyle>, wnd: Rc<Window>) {
    if self.is_show() {
      return;
    }
    let this = self.clone();
  ...
  fn wrap_style(&self, w: impl FnWidget, style: OverlayStyle) -> impl FnWidget {
    let this = self.clone();
    fn_widget! {
