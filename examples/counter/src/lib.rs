use ribir::{material::DisabledRipple, prelude::*};
...
pub fn run() {
  ...
  App::run(list! {
    ...
    @ListItem {
      ...
      @ListItemHeadline { @ { "Only Headline"} }
      @ListItemTrailingSupporting { @ { "100+"} }
      @Trailing { @Checkbox { providers: [Provider::new(DisabledRipple(true))] } }
    }
    @ListItem {
      interactive: true,
      @Avatar { @ { "A" } }
    ...
    @ListItem {
      interactive: true,
      supporting_lines: 2usize,
      @ListItemThumbNail {
        @Container {
          size: Size::new(160., 90.),
          background: Color::GREEN
        }
      }
      @ListItemHeadline { @ { "Counter"} }
      @ListItemSupporting { @ { "there is supporting lines, many lines, wrap to multiple lines, xxhadkasda"} }
      // @ListItemTrailingSupporting { @ { "100+" } }
      @Trailing { @Checkbox {} }
    }
  })
