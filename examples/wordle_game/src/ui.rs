...
trait WordleExtraWidgets: StateWriter<Value = Wordle> + Sized {
  ...
  fn char_key(&self, key: char) -> impl IntoWidget<FN> {
  ...
  fn keyboard(&self, state_bar: impl StateWriter<Value = Text>) -> impl IntoWidget<FN> {
    let this = self.clone_writer();
    fn_widget! {
  ...
  fn chars_grid(&self) -> impl IntoWidget<FN> {
    let this = self.clone_writer();
    fn_widget! {
      @Column {
        item_gap: 5.,
