...
impl Wordle {
  fn chars_key<const N: usize>(
    this: &Writer<Wordle>, chars: [char; N],
  ...
  fn char_key(this: &Writer<Wordle>, key: char) -> Widget<'static> {
  ...
  fn keyboard(this: Writer<Wordle>, state_bar: Stateful<Text>) -> Widget<'static> {
    let palette = Palette::of(BuildCtx::get());
  ...
  fn chars_grid(this: &Writer<Wordle>) -> Widget<'static> {
    fn_widget! {
      @Flex {
        direction: Direction::Vertical,
        item_gap: 5.,
