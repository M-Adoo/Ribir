...
fn rail_root_init(classes: &mut Classes) {
  classes.insert(
    NAVIGATION_RAIL,
    style_class! {
      height: Measure::Percent(1.),
      background: Palette::of(BuildCtx::get()).surface_container()
    },
  );

  classes.insert(NAVIGATION_RAIL_EXPANDED, style_class! { min_width: 220., max_width: 360. });
  classes.insert(NAVIGATION_RAIL_COLLAPSED, style_class! { width: 96. });
